...
public class CohenSutherland {
	/*
	 * Convention: p1,p2 are the vertices of the clipped edge. cp1, cp2 are the
	 * vertices giving the clipping region
	 */
	...
	public static boolean whollyOutside(Point p1, Point p2, Point cp1, Point cp2) {
	...
	/*
	 * Determines if line(p1,p2) intersects in any way rect(cp1,cp2). This is
	 * actually the first half of the Cohen-Sutherland algorithm; the second
	 * half would be adjusting the edges to fit inside the clip rectangle, which
	 * isn't germane to our purposes.
	 *
	 ...
	 * bitwise-AND of each vertex of an edge indicates whether the line lies
	 * entirely inside (0 & 0 == 0), entirely outside (nonzero & nonzero2) ==
	 * nonzero3), or if it crosses the clipping rectangle (any other
	 * possibility).
	 */
	public static boolean clips(Point p1, Point p2, Point cp1, Point cp2) {
		...
		if (p1code == 0 && p2code == 0)
			...
			r = true;
		if (false)
			System.out.println("ch clip: edge={" + "(" + p1.x + "," + p1.y + ")," + "(" + p2.x + "," + p2.y + ")}" + " box={" + "(" + cp1.x + "," + cp1.y + ")," + "(" + cp2.x + "," + cp2.y + ")}"
					+ "} code={" + p1code + "," + p2code + "}, isect=" + icode + " clips=" + r);
		return r;
	}

	/*
	 * Computes the 4-bit code for a point p with respect to line(p1,p2)
	 */
	...
	public static int below(Point p1, Point p2, Point p) {
		if (p.y > Math.max(p1.y, p2.y)) {
			if (false)
				System.out.println("ch.below*: comparing " + p.y + " vs " + Math.max(p1.y, p2.y));
			return 0x4;
		}
		return 0;
	}
